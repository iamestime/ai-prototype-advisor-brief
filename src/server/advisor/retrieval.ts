...
export type Chunk = { id: string; sectionId: string; form: string; item: string; filingDate: string; accession: string; url: string; fetchedAt: string; index: number; text: string };
...
export function chunkSections(sections: Section[]): Chunk[] {
  ...
  for (const s of sections) {
    ...
    while (i < text.length) {
      ...
      if (piece.length > 40) out.push({ id: `${s.id}#${n}`, sectionId: s.id, form: s.form, item: s.item, filingDate: s.filingDate, accession: s.accession, url: s.url, fetchedAt: s.fetchedAt, index: n, text: piece });
      n++;
...
const STOP = new Set("the a an and or of to in for on with by as at from that this is are was were be been it its their our we they has have had not no".split(" "));
...
export async function buildIndex(cfg: Cfg, key: string, sections: Section[]): Promise<VectorIndex> {
  ...
  // Embed in batches of 64; a failure on any batch means lexical mode for the whole index (never a partial vector index).
  if (cfg.providers.length) {
    const all: Float32Array[] = [];
    let ok = true;
    for (let i = 0; i < chunks.length && ok; i += 64) {
      const batch = chunks.slice(i, i + 64).map((c) => c.text);
      const r = await embed(cfg, batch, cfg.EMBED_TIMEOUT_MS);
      if (!r || r.vectors.length !== batch.length) { ok = false; break; }
      for (const v of r.vectors) all.push(normalize(Float32Array.from(v)));
      embedModel = r.model; provider = r.provider;
    }
    if (ok && all.length === chunks.length) vectors = all;
    else { embedModel = null; provider = null; }
  }
  return { key, chunks, vectors, mode: vectors ? "hybrid" : "lexical", embedModel, provider, builtAt: new Date().toISOString(), buildMs: Date.now() - t0, df, avgLen, tokens };
}
...
function bm25(index: VectorIndex, query: string[]): number[] {
  const N = index.chunks.length, k1 = 1.4, b = 0.75;
...
export async function search(cfg: Cfg, index: VectorIndex, question: string, k = 6): Promise<{ hits: Hit[]; mode: "hybrid" | "lexical" }> {
  ...
  if (index.vectors) {
    ...
    if (r?.vectors[0]) {
      ...
      vec = index.vectors.map((v) => { let s = 0; for (let i = 0; i < v.length; i++) s += v[i]! * q[i]!; return s; });
    }
  }
