...
// Intelligence: Anthropic Messages API with prompt caching, strict JSON blocks, citation validation.
// Secrets: ANTHROPIC_API_KEY (required), ANTHROPIC_MODEL (optional), EDGAR_USER_AGENT (recommended).
...
let CFG: Cfg = {
  ...
  EDGAR_UA: "AdvisorBrief prototype contact@example.com",
  PRICE_IN: 3,
  PRICE_OUT: 15,
...
function readEnv(): Cfg {
  ...
  return {
    ...
    ANTHROPIC_MODEL: e["ANTHROPIC_MODEL"] ?? "claude-sonnet-4-5",
    EDGAR_UA: e["EDGAR_USER_AGENT"] ?? "AdvisorBrief prototype contact@example.com",
    PRICE_IN: Number(e["PRICE_INPUT_PER_MTOK"] ?? 3),
    PRICE_OUT: Number(e["PRICE_OUTPUT_PER_MTOK"] ?? 15),
    PRICE_CACHE_READ: Number(e["PRICE_CACHE_READ_PER_MTOK"] ?? 0.3),
  };
}
...
async function resolve(query: string): Promise<Company> {
  ...
  const rows = Object.values(table as object) as Array<{ cik_str: number; ticker: string; title: string }>;
  ...
  const hits = rows
    ...
    .sort(
      (a, b) =>
        Number(!a.title.toLowerCase().startsWith(ql)) - Number(!b.title.toLowerCase().startsWith(ql)) ||
...
const companyFacts = (cik: number) =>
  cached(`facts_${cik}`, 3600e3, () => edgarGet(`https://data.sec.gov/api/xbrl/companyfacts/CIK${String(cik).padStart(10, "0")}.json`));
...
function quarterlySeries(facts: any, tags: string[], units = ["USD"], quarters = 8) {
  ...
  return {
    ...
    points: keys.map((k) => {
      const [yr, qq] = k.split("-");
      const v = q.get(k);
      return { frame: `CY${yr}Q${qq}`, label: `Q${qq} ${yr}`, periodEnd: v.end, value: v.val, form: v.form, filed: v.filed, derived: !!v.derived };
    }),
  };
}
...
function financials(facts: any) {
  ...
  const sharesOutstanding = best
    ? { tag: "EntityCommonStockSharesOutstanding", value: best.val, asOf: best.end, filed: best.filed, form: best.form }
    : null;
...
async function getQuote(ticker: string) {
  ...
  try {
    const r = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(ticker)}?range=5d&interval=1d`, {
      headers: { "User-Agent": "Mozilla/5.0 (AdvisorBrief prototype)" },
    });
...
function derived(quote: any, fin: any) {
  ...
  if (price && shares) {
    ...
    out.marketCapBasis = `price x ${shares.toLocaleString()} shares (dei:EntityCommonStockSharesOutstanding as of ${fin.sharesOutstanding.asOf})`;
  }
  if (price && fin.ttmEps) {
    out.trailingPE = fin.ttmEps > 0 ? price / fin.ttmEps : null;
    out.ttmEps = fin.ttmEps;
  }
  if (price && fin.ttmDividend != null) {
    out.dividendYield = fin.ttmDividend / price;
    out.ttmDividend = fin.ttmDividend;
  }
  return out;
}
...
const EIGHT_K_ITEMS: Record<string, string> = {
  ...
  "5.02": "Officer or director change, compensation",
...
function sections10k(html: string, f: Filing): Section[] {
  ...
  const out: Section[] = [];
  for (const item of ["Item 1", "Item 1A", "Item 7"]) {
    const b = extractItem(text, item);
    if (b) out.push(section("10-K", f, item, ITEM_TITLES[item]!, b, item, { reportDate: f.reportDate }));
  }
...
5. Output only the JSON object requested. No markdown fences, no commentary.`;

const BLOCKS: Record<string, { title: string; prompt: string }> = {
  summary: {
    title: "60 second summary",
    prompt: `Write the 60 second summary an advisor reads before the call. Four to five sentences: what the company does, how it makes money, how the most recent quarter went in the filing's own terms, and the one thing management says it is focused on.\nReturn: {"paragraphs": [{"text": "...", "citations": ["<section id>", ...]}]} with two or three paragraphs.`,
  },
  what_changed: {
    title: "What changed since last quarter",
    prompt: `From the most recent 10-Q MD&A (and the 10-K for comparison), list the four to six most important changes an advisor should know: growth drivers, margin moves, guidance or outlook language, capital return, balance sheet, segment shifts. Each item one or two sentences with the specific figures the filing gives.\nReturn: {"items": [{"text": "...", "citations": ["<section id>", ...]}]}`,
  },
  risks: {
    title: "Top risks",
    prompt: `From the risk factor sections, pick the four or five risks that matter most to a long term shareholder right now, ranked. Prefer risks that are specific to this company over generic boilerplate. Title of five words or fewer, then two sentences in plain English on why it matters.\nReturn: {"items": [{"title": "...", "severity": "high|medium|low", "text": "...", "citations": ["<section id>", ...]}]}`,
  },
  events: {
    title: "Recent 8-K events",
    prompt: `For every 8-K section provided, write one timeline entry: the filing date, a headline of ten words or fewer, and one or two sentences on why an advisor would care. If an 8-K is routine (earnings release notice, exhibit only), say so in one short sentence. Newest first. If there are no 8-K sections, return an empty list.\nReturn: {"items": [{"date": "YYYY-MM-DD", "headline": "...", "why_it_matters": "...", "citations": ["<section id>"]}]}`,
  },
  talking_points: {
    title: "Talking points for the client conversation",
    prompt: `Write five talking points an advisor can say out loud to a client today. Each one sentence, conversational, specific, grounded in the filings, no jargon. Cover the business, the latest quarter, a risk to acknowledge, capital return or balance sheet, and what to watch next quarter.\nReturn: {"items": [{"text": "...", "citations": ["<section id>", ...]}]}`,
  },
  questions: {
    title: "Questions the client may ask",
    prompt: `List four questions a sharp client is likely to ask about this company, each with a two sentence answer the advisor can give from the filings. Include at least one uncomfortable question.\nReturn: {"items": [{"question": "...", "answer": "...", "citations": ["<section id>", ...]}]}`,
  },
};
  ...
  toDict() {
    return {
      ...
      estimatedCostUsd: Number(
        (
          (this.input * CFG.PRICE_IN + this.cacheWrite * CFG.PRICE_IN * 1.25 + this.cacheRead * CFG.PRICE_CACHE_READ + this.output * CFG.PRICE_OUT) /
          1e6
        ).toFixed(4),
      ),
...
async function callBlock(context: string, name: string, valid: Set<string>, usage: Usage): Promise<any> {
  const messages: any[] = [
    {
      role: "user",
      content: [
        { type: "text", text: context, cache_control: { type: "ephemeral" } },
        { type: "text", text: BLOCKS[name]!.prompt },
      ],
    },
  ];
  let lastErr = "";
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      const [block, dropped] = validateCitations(parseJson(text), valid);
      block.droppedCitations = dropped;
      return block;
    } catch (e) {
      lastErr = String(e);
      messages.push({ role: "assistant", content: text }, { role: "user", content: "That was not valid JSON. Return only the JSON object." });
    }
  }
  return { items: [], error: `model returned invalid JSON twice: ${lastErr}` };
}
    ...
    async start(controller) {
      ...
      try {
        ...
        if (!CFG.ANTHROPIC_API_KEY) {
          send("error", { message: "ANTHROPIC_API_KEY is not set. Add it as a backend secret to generate briefings." });
          return;
        }
        send("status", { message: "Reading filings with Claude" });
