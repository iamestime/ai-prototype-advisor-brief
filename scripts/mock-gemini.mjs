...
function words(text, count = 16) {
  return text.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().split(" ").slice(0, count).join(" ");
}
...
function researchBlocks(user) {
  ...
  return names.map((name) => {
    if (name === "summary") {
      return { block: name, paragraphs: [{ text: "The latest filing describes the company, its operating model, and recent performance.", citations: [sectionId] }] };
    }
    if (name === "risks") {
      return { block: name, items: [{ title: "Execution risk", severity: "medium", text: "The filing identifies operating risks that merit continued attention.", citations: [sectionId] }] };
    }
    if (name === "events") return { block: name, items: [] };
    if (name === "questions") {
      return { block: name, items: [{ question: "What should the client watch?", answer: "Watch the operating factors management discusses in the latest filing.", citations: [sectionId] }] };
    }
    return { block: name, items: [{ text: "The filing provides a current, source-linked point for the client conversation.", citations: [sectionId] }] };
  });
}
...
function answerFor(body) {
  ...
  if (system.includes("wealth management advisor's question")) {
    ...
    return JSON.stringify({
      answer: "The retrieved filing passage addresses the company's operations and recent performance.",
      citations: [id],
  ...
  return researchBlocks(user).map((block) => JSON.stringify(block, null, 2)).join("\n");
}
...
const server = createServer(async (request, response) => {
  ...
  if (body.stream) {
    ...
    for (let i = 0; i < content.length; i += 53) {
      response.write(`data: ${JSON.stringify({ choices: [{ delta: { content: content.slice(i, i + 53) } }] })}\n\n`);
    }
    response.write(`data: ${JSON.stringify({ choices: [], usage: { prompt_tokens: 120, completion_tokens: 60 } })}\n\n`);
